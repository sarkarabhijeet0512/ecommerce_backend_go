...
type GinJWTMiddleware struct {
	...
	// Callback function that should perform the authentication of the user based on signup info.
	// Must return user data as user identifier, it will be stored in Claim Array. Required.
	// Check error (e) to determine the appropriate error message.
	AuthenticatorSignup func(c *gin.Context) (interface{}, error)
	...
	// User can define own SignupResponse func.
	SignupResponse func(*gin.Context, int, string, time.Time)
...
var (
	...
	// ErrMissingAuthenticatorsignupFunc indicates Authenticatorsignup is required
	ErrMissingAuthenticatorsignupFunc = errors.New("ginJWTMiddleware.Authenticatorsignup func is undefined")

	// ErrMissingLoginValues indicates a user tried to authenticate without username or password
	ErrMissingLoginValues = errors.New("missing Username or Password")

	// ErrMissingSigninValues indicates a user tried to authenticate without username or password
	ErrMissingSigninValues = errors.New("missing Username or Password")

	// ErrFailedAuthentication indicates authentication failed, could be faulty username or password
	ErrFailedAuthentication = errors.New("incorrect Username or Password")

	// ErrFailedAuthentication indicates authentication failed, could be faulty username or password
	ErrOTPNotVerified = errors.New(" error in otp verification")
	...
	ErrInvalidSigningAlgorithm = errors.New("please login")
	...
	//ErrUserNotFound when mobile no is not found in database
	ErrUserNotFound = errors.New("user not found. Please create a new account")

	//ErrIncorrectOTP when mobile no is not found in database
	ErrIncorrectOTP = errors.New("incorrect otp, please try again")
)
...
func (mw *GinJWTMiddleware) MiddlewareInit() error {
	...
	if mw.SignupResponse == nil {
		mw.SignupResponse = func(c *gin.Context, code int, token string, expire time.Time) {
			c.JSON(http.StatusOK, gin.H{
				"code":   http.StatusOK,
				"token":  token,
				"expire": expire.Format(time.RFC3339),
...
func FetchTimeout() time.Duration {
	...
	// }
	logoutTimeout := time.Hour * 24 * 30
	return logoutTimeout
}

// SignupHandler can be used by clients to get a jwt token.
// Payload needs to be json in the form of {"username": "USERNAME", "password": "PASSWORD"}.
// Reply will be of the form {"token": "TOKEN"}.
func (mw *GinJWTMiddleware) SetToken(c *gin.Context, data interface{}) (int, string, time.Time) {
	tOut := FetchTimeout()
	mw.Timeout = tOut
	mw.CookieMaxAge = tOut
	// Create the token
	...
	log.Println("time from func: ", mw.TimeFunc())
	log.Println("timeout: ", mw.Timeout)
	...
	return http.StatusOK, tokenString, expire
}
...
func (mw *GinJWTMiddleware) LoginHandler(c *gin.Context) {
	...
	if err != nil {
		if err == ErrFailedAuthentication {
			mw.unauthorized(c, http.StatusUnauthorized, mw.HTTPStatusMessageFunc(err, c))
			return
		} else {
			mw.unauthorized(c, http.StatusUnprocessableEntity, mw.HTTPStatusMessageFunc(err, c))
			return
		}
	}
	tOut := FetchTimeout()
...
func (mw *GinJWTMiddleware) jwtFromHeader(c *gin.Context, key string) (string, error) {
	...
	// log.Println("header cookie: ", key, authHeader)
	if authHeader == "" {
...
func (mw *GinJWTMiddleware) jwtFromQuery(c *gin.Context, key string) (string, error) {
	token := c.Query(key)
	// log.Println("query cookie: ", key, token)
	if token == "" {
...
func (mw *GinJWTMiddleware) jwtFromCookie(c *gin.Context, key string) (string, error) {
	cookie, e := c.Cookie(key)
	log.Println("error while fetching cookie: ", e)
	if cookie == "" {
