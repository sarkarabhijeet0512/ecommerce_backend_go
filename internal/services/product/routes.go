...
func v1Routes(router *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware, awsSession *session.Session, o *Options) {
	...
	// add new routes here
	r.PUT("/product_details", mw.AWSSessionAttach(awsSession), authMiddleware.MiddlewareFunc(), o.ProductDetailsHandler.UpsertProductDetails)
	r.GET("/product_details/:product_id", authMiddleware.MiddlewareFunc(), o.ProductDetailsHandler.GetProductDetails)
	r.GET("/product/catergory/:category_id", authMiddleware.MiddlewareFunc(), o.ProductDetailsHandler.GetProductListByCategory)
	r.GET("/product/catergory", authMiddleware.MiddlewareFunc(), o.ProductDetailsHandler.GetProductCategory)
